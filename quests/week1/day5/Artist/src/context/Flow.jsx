  ...
  const fetchListings = useCallback(
    async () => {
      const listings = await fcl.send([
        fcl.script`
          import LocalArtistMarket from 0x255810653690cb4c
  
          pub fun main(): [LocalArtistMarket.Listing] {
            let account = getAccount(0x255810653690cb4c)
      ...
      const transactionId = await fcl.send([
        fcl.transaction`
          ...
          transaction(pixels: String, price: UFix64) {
            ...
            prepare(account: AuthAccount) {
              // TODO: Change to your contract account address.
              self.marketRef = getAccount(0x255810653690cb4c)
                .getCapability(/public/LocalArtistMarket)
                .borrow<&{LocalArtistMarket.MarketInterface}>()
      ...
      const transactionId = await fcl.send([
        fcl.transaction`
          ...
          transaction(listingIndex: Int) {
            prepare(account: AuthAccount) {
              let marketRef = getAccount(0x255810653690cb4c)
                .getCapability<&{LocalArtistMarket.MarketInterface}>(/public/LocalArtistMarket)
                .borrow()
                ?? panic("Couln't borrow market ref")
        ...
        fcl.transaction`
          ...
          transaction(listingIndex: Int) {
            prepare(account: AuthAccount) {
              ...
              let vault = account.borrow<&FungibleToken.Vault>(from: /storage/flowTokenVault)!
              marketRef.buy(
                listing: listingIndex,
                with: <- vault.withdraw(amount:listing.price),
