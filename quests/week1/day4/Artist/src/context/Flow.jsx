  ...
  const createCollection = useCallback(
    async () => {
        ...
        const transactionId = await fcl.send([
          fcl.transaction`
            import LocalArtist from 0x255810653690cb4c
            transaction() {
              prepare(account: AuthAccount) {
                var ref = account.borrow<&{LocalArtist.PictureReceiver}>(from: /storage/LocalArtistPictureCollection)
                if ref == nil {
                  ...
                  log("duplicated collection creat.")
                }
              }
            }
          `,
    ...
    async (address) => {
      if (address || state.user.addr) {
        try {
          ...
          console.log("Start Fectch collection");
          ...
          const collection = await fcl.send([
            fcl.script`
              ...
              pub fun main(address:Address):[LocalArtist.Canvas] {
                ...
                let pictureReceiverRef = account.getCapability<&{LocalArtist.PictureReceiver}>(/public/LocalArtistPictureReceiver).borrow()
                  ?? panic("Couldn't borrow picture receiver ref")
          ...
          console.log("get collections:", collection)
        ...
        .send([
          fcl.transaction`
            ...
            transaction(width: Int, height: Int, pixels: String) {
              ...
              prepare(account: AuthAccount) {
                let printerRef = getAccount(0x255810653690cb4c)
                  .getCapability<&LocalArtist.Printer>(/public/LocalArtistPicturePrinter).borrow()
                  ?? panic("Couln't borrow printer ref")
                self.picture <- printerRef.print(width: width, height: height, pixels: pixels)
                self.collectionRef = account
                  .getCapability<&{LocalArtist.PictureReceiver}>(/public/LocalArtistPictureReceiver).borrow()
                  ?? panic("Couldn't borrow picture receiver ref.")
              }
              execute {
                if self.picture == nil {
